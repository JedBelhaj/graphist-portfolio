...
import { HERO_ARROW, HERO_PERSON, HERO_STICKERS } from "@/lib/content";

/* Entrance sequence, in ms: the left column rises first, then she slides in
   from the right, then the burst behind her, then the logos pop one by one. */
const T_PERSON = 100;
const T_BURST = 800;
const T_LOGOS = 1250;
  ...
  return (
    <section className="relative overflow-hidden bg-[rgb(254,254,254)] pt-28 lg:pt-0">
      <div className="mx-auto grid max-w-[1300px] grid-cols-1 items-center gap-10 px-5 sm:px-8 lg:min-h-screen lg:grid-cols-2 lg:gap-6 lg:px-[43px]">
        ...
        <div className="order-1 lg:order-2 lg:hidden">
          <div className="relative mx-auto flex max-w-md items-end justify-center overflow-hidden rounded-3xl bg-[rgb(255,218,223)] pt-8">
            <img src={HERO_PERSON} alt="Marissa" className="hero-rise relative z-[5] max-h-[420px]" />
          </div>
        </div>
      </div>
      ...
      <div className="absolute inset-y-0 right-0 hidden w-[42%] items-end justify-center bg-[rgb(255,218,223)] lg:flex">
        <div className="relative left-[-30%] flex items-end justify-center">
          <img
            src={HERO_PERSON}
            alt="Marissa"
            className="hero-in-right relative z-[5] max-h-[600px]"
            style={{ animationDelay: `${T_PERSON}ms` }}
          />
          {HERO_STICKERS.map((s, i) => {
            // index 0 is the burst behind her head; the rest are the tool logos
            const isBurst = i === 0;
            return (
              <div
                key={i}
                className={`absolute ${s.cls} ${isBurst ? "hero-in-right" : "hero-pop"}`}
                style={{
